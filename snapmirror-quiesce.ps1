...
function main {
    try {
        ...
        $snapmirrorQuiesceFolderPath = Join-Path -Path $logsFolderPath -ChildPath "snapmirror-quiesce"
        if (-not (Test-Path -Path $snapmirrorQuiesceFolderPath)) {
            New-Item -Path $snapmirrorQuiesceFolderPath -ItemType Directory | Out-Null
        }
    
        # Generate the log file name based on the variable value and the current timestamp
        $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
        $logFileName = "logs`_$timestamp.log"
        $global:logFilePath = Join-Path -Path $snapmirrorQuiesceFolderPath -ChildPath $logFileName
...
function snapmirrorQuiesce {
    ...
    for($i = 0; $i -lt $DESTINATION_VOLUME_NAMES.Length; $i++) {
        try {
            ...
            Invoke-NcSnapmirrorQuiesce -DestinationLocation "$($DESTINATION_VSERVER):$($DESTINATION_VOLUME_NAMES[$i])" -ErrorAction Stop
